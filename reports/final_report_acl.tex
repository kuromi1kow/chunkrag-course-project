...
improvements as the primary path to higher scores.
\end{abstract}
...
placement is the main bottleneck once retrieval is only partially successful.
...
\textbf{Generator alignment interacts with chunking quality.} Prompt design can move
end-to-end QA quality substantially without changing the retriever or chunking strategy.
Tightening the extractive QA instruction to encourage short grounded spans produced
large quality gains independent of chunking choice, and once this was in place,
recursive chunking emerged as the most robust setting across both datasets. This
interaction (prompt quality and chunking quality are not independent) implies
that chunking comparisons in the literature may partly reflect differences in how each
chunking style interacts with a given prompt, rather than purely retrieval effects.
...
One annotated example per failure type from \texttt{recursive\_256}, drawn from
whichever dataset provides the clearest illustration. Failure types follow the taxonomy
defined in Section~3.4; predictions are verbatim.
...
\paragraph{Terse extraction (SQuAD)}
\textit{Q:} How many refugees emigrated to the Dutch Republic?\\
\textit{Gold:} 75,000 to 100,000 \quad
\textit{Pred:} 75,000\\
\textit{F1:} 0.50 \quad \textit{Recall@4:} 1.00\\[2pt]
The prediction is a strict subset of the gold tokens: the model gave the lower bound of
the range but omitted the upper bound. The answer is partially correct but incomplete.

\paragraph{Close paraphrase (SQuAD)}
\textit{Q:} Contracts must be designed to ensure what?\\
\textit{Gold:} that each side is capable of performing the obligations set out\\
\textit{Pred:} Contracts must ensure each side is capable of performing obligations\\
\textit{F1:} 0.74 \quad \textit{Recall@4:} 1.00\\[2pt]
The answer is semantically equivalent but uses a shorter paraphrase that drops ``that''
and ``set out''. Neither prediction nor gold is a token-set subset of the other, so the
case escapes both verbose and terse buckets. Only a looser evaluation metric would
credit it.

\paragraph{Partial answer (SQuAD)}
\textit{Q:} What aspect of Western medicine did the Chinese dislike?\\
\textit{Gold:} its humoral system \quad
\textit{Pred:} Chinese physicians opposed Western medicine due to its contradiction
with yin-yang and wuxing philosophy\\
\textit{F1:} 0.12 \quad \textit{Recall@4:} 1.00\\[2pt]
The model produced a lengthy response that touches on the right topic but names the
wrong reason. The only common token after normalization is ``its'', yielding a near-zero
F1 despite full retrieval success.
